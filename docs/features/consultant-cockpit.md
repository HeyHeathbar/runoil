...
- **Consumption chat** (Truth/Open mode, model picker, voice) → **BYO keys**: runs on the active tenant's configured provider keys; user-initiated, so the tenant's model choice and bill are intuitive.
...
- **Residency escape hatch (Governed tier):** for clients who contractually require all data to flow through their own provider account, engine calls may be routed through the client's key — but RunOil still **pins the model version and prompts**, so only the billing/account path changes and evals remain valid. Opt-in, not default.
...
- **Residency escape hatch — scope:** confirm which provider accounts the Governed-tier engine-routing supports (e.g. Azure OpenAI, Bedrock) and the contract trigger for enabling it.
