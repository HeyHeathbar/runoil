...
| | |
|---|---|
| **Product** | RunOil — an organizational intelligence platform for the AI era |
...
| **Go-to-market** | Channel-led: independent AI consultants deliver; RunOil owns the client (Shopify model) |
...
It is sold to mid-market companies and delivered through a channel of independent AI consultants, on a Shopify-style model: **RunOil owns the client relationship and the data**; consultants are a compensated, revocable channel. Defensibility is ERP-style — data gravity, accumulated configuration, audit and governance, and compounding switching cost — not features. It is built to be run as a company of one: the founder plus AI agents, with partners doing the hands-on client delivery.
...
**RunOil owns the client.** The client company is the customer of record — it pays the subscription and owns its Corpus. Consultants are the channel that does the selling and delivery; RunOil owns the billing, the data, and the renewal. The partner does the direct touch, which is what makes a company of one viable.

### Consultant compensation (Shopify model)
1. **Recurring referral commission.** A recurring percentage (20% is the Shopify anchor) of the subscription of clients the consultant refers, paid only on active paying clients, attributed via a partner-initiated provisioning/transfer of the instance.
2. **Services billed directly.** Discovery sessions, implementation, and ongoing review-queue adjudication are billed by the consultant to the client; RunOil takes no cut. This is the consultant's primary, recurring income.

**Billing stays separate.** RunOil bills the subscription; the consultant bills services. RunOil never becomes the payment intermediary for consultant services.
